...
//	    
//	    // 현재 시간을 원하는 형식으로 문자열로 변환 (예: "yyyy-MM-dd HH:mm:ss")
//	    DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm");
//	    String enterTime = currentTime.format(formatter);
//	    Map<String, Object>param = new HashMap<>();
//	    param.put("idx", idx);
//	    param.put("enterTime", enterTime);
		...
		//int memberIdx=Integer.parseInt(session.getHandshakeHeaders().getFirst("memberIdx"));
...
//		 int memberIdx = Integer.parseInt(memberIdxObject.toString());
	    ...
	    String jsonMessage = objectMapper.writeValueAsString(preNenterMsg);
...
//		//JSON 메시지를 ChatMsg 객체로 변환
		...
		int result = chatRoomService.msgSave(chatMsg);
			if(result>0) {
				String responsePayload = objectMapper.writeValueAsString(chatMsg);
